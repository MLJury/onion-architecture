...
IF EXISTS(SELECT 1 FROM sys.procedures WHERE [object_id] = OBJECT_ID('req.spDeleteEducationDegree'))
	DROP PROCEDURE req.spDeleteEducationDegree
GO

CREATE PROCEDURE req.spDeleteEducationDegree
...
BEGIN
	...
	BEGIN TRY
		BEGIN TRAN
			...
			DELETE FROM req.EducationDegree
			WHERE ID = @ID
